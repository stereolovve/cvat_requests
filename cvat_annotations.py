...
def get_jobs(
    cookies, project_id=None, task_id=None, job_id=None, assignee=None, status=None
):
    url = f"{BASE_URL}/jobs"
    params = {}
...
def main():
    ...
    for i, job in enumerate(jobs, 1):
        ...
        assignee_username = (
            assignee.get("username", "unassigned") if assignee else "unassigned"
        )

        task_name = get_task_name(cookies, task_id) if task_id else "Unknown"

        print(
            f"Processing job {i}/{len(jobs)}: job_id={job_id}, task_id={task_id}, task_name='{task_name}'"
        )
    ...
    for job in jobs_data:
        print(
            f"Job {job['job_id']:<5} | Task: {job['task_name']:<30} | "
